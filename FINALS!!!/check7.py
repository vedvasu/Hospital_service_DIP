        ...
        for z2 in range (x,y):                                          #loop controling the next step in a path->valid or not
            ...
            for z3 in range(0,y):                                       #for checking the redundancy of any element previously occured in the path
                if check[z3]==(cxa,cya):
                    ...
                    t4=1
            if (cxa<400)&(cya<320):                                     #conditions testing if next step is valid or not
...
def get_perspective_image(frame):
    ...
    pts2 = np.float32([[0,0],[0,320],[400,0],[400,320]])
    persM = cv2.getPerspectiveTransform(pts1,pts2)
    img = cv2.warpPerspective(frame,persM,(400,320))                                # resolution set to 400x400.....we are processing image on this resolution
    return img
